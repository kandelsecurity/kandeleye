...
import { Layers, BarChart3, Newspaper, Search, X, Globe, MapPinned, Route, Radar, Satellite, Moon, ExternalLink, AlertTriangle, Activity, Database, Wifi, Play, Network, Crosshair, Bluetooth, Pentagon, Radio , PenLine, Shield, Zap, WifiOff, Cpu, MapPin } from 'lucide-react';
...
export default function Dashboard() {
  ...
  /* ── KANDEL INTEGRATION ── */
  const [kandelData, setKandelData] = useState<{ vulnerabilities: any[]; wifiNetworks: any[]; networkDevices: any[]; gpsPoints: any[]; summary: any }>({ vulnerabilities: [], wifiNetworks: [], networkDevices: [], gpsPoints: [], summary: null });
  ...
  useEffect(() => { const iv = setInterval(pollKandelData, 5000); return () => clearInterval(iv); }, [pollKandelData]);
  ...
  useEffect(() => {
    (window as unknown as { kandeleyeWatchFlight?: (f: WatchedFlight) => void }).kandeleyeWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const handleExportGeoJSON = useCallback(() => {
    downloadFile(
      `kandeleye-aoi-${new Date().toISOString().slice(0, 10)}.geojson`,
  ...
  // ── KANDELeye SDK — Intelligence Fusion Layer ──
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── KANDELeye title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'KANDELeye'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      {/* ── MAP — ROSA KANDEL ── */}
      <div className="kandel-pink-map" style={{ filter: 'hue-rotate(300deg) saturate(1.4) brightness(1.05) contrast(1.1)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-[3px] p-[3px] pointer-events-auto rounded-xl border border-[var(--border-primary)] bg-[var(--bg-panel)] backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.55)]">
          ...
          <button onClick={() => setShowAuditPanel(p => !p)} title="Panel KANDEL" aria-label="Panel KANDEL" className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-mono font-medium tracking-[0.15em] text-[var(--gold-light)] hover:text-[var(--gold-primary)] hover:bg-[var(--gold-primary)]/10 transition-all duration-200">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          <img src="/logo-kandel.png" alt="KANDEL" className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-lg object-contain drop-shadow-[0_0_12px_rgba(138,43,226,0.6)] border border-[#8A2BE2]/20" />
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#8A2BE2] font-mono">KANDELeye</h1>
            <span className="text-[9px] md:text-[10px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#8A2BE2]">KANDEL SECURITY × DARK OPS</span>
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-3 text-[10px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <a href='https://ko-fi.com/M8D41ZYW4Z' target='_blank' rel='noopener noreferrer' className="pointer-events-auto glass-panel px-3 py-1.5 flex items-center gap-1.5 text-[9px] font-mono tracking-widest hover:opacity-80 transition-opacity border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10 ml-3 shadow-[0_0_10px_rgba(255,215,0,0.1)]">
          <div className="w-1.5 h-1.5 rounded-full bg-[var(--gold-primary)] animate-kandeleye-pulse" />
          <span className="text-[var(--gold-primary)] font-bold">KANDEL SECURITY</span>
        </a>
      </motion.div>
      ...
      {isMobile && !showDirections && !navSession && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute top-3 right-3 z-[200] pointer-events-auto flex items-center gap-2">
          ...
          <a href='https://ko-fi.com/M8D41ZYW4Z' target='_blank' rel='noopener noreferrer' className="glass-panel px-2 py-1 flex items-center gap-1.5 text-[9px] font-mono tracking-widest hover:opacity-80 transition-opacity border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10">
            <div className="w-1 h-1 rounded-full bg-[var(--gold-primary)] animate-kandeleye-pulse" />
            <span className="text-[var(--gold-primary)] font-bold">KANDEL SECURITY</span>
          </a>
        </motion.div>
      )}
            ...
            {showArcGIS && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="absolute right-12 top-1/2 -translate-y-1/2 w-[340px]">
                <div className="glass-panel p-3 max-h-[70vh] overflow-y-auto styled-scrollbar">
                  <ArcGISPanel
                    onImportLayer={(layer) => setArcgisLayers(prev => [...prev.filter(l => l.id !== layer.id), { ...layer, color: layer.color || '#8A2BE2', visible: true, opacity: layer.opacity ?? 0.8 }])}
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-kandeleye-pulse" />
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[10px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'KANDELeye RECON' : mobilePanel === 'remote' ? 'WORLD REMOTE' : 'SEARCH'}
                    </span>
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 kandeleye-glow">
            <div className="flex items-center justify-between mb-3">
