...
const FALLBACK_ORIGIN = 'https://kandeleye.live';
    ...
    <div className="docs-root min-h-screen bg-[var(--bg-void)] text-[var(--text-primary)] antialiased">
      ...
      <header className="sticky top-0 z-[300] border-b border-white/[0.06] bg-[var(--bg-void)]/85 backdrop-blur-xl">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-14 flex items-center gap-3">
          ...
          <a
            href="https://github.com/kandelsecurity/kandeleye"
            target="_blank"
        ...
        <main ref={mainRef} className="flex-1 min-w-0 py-12 lg:py-16 max-w-3xl">
          ...
          <div className="mb-20">
            ...
            <p className="text-[15px] leading-[1.75] text-[var(--text-secondary)] max-w-[42rem]">
              KANDELeye aggregates aviation, maritime, seismic, conflict, cyber, and OSINT feeds onto a single
...
#   "timestamp": "2026-07-29T12:00:00Z" }`}</Pre>
          ...
          <Section id="self-hosting" eyebrow="Guide" title="Self-Hosting">
            <p>KANDELeye needs Node 20+ and no database. A local instance is three commands:</p>
            <Pre label="Local development" lang="bash">{`git clone https://github.com/kandelsecurity/kandeleye.git
cd osiris
npm install
...
npm run test:live  # includes tests that hit live upstream feeds`}</Pre>
            <p>
              ...
              always listens on port 3000 internally; <Code>KANDELeye_PORT</Code> controls the host port it is published
              on.
            </p>
...
docker compose up -d`}</Pre>
              ...
              {[
                {
                  ...
                  v: 'Points at the separate RECON scanner backend. SCANNER_KEY must equal that backend’s KANDELeye_KEY. Leave both empty to disable RECON — /api/scanner then returns 503 by design.',
                },
                {
                  k: 'SDK_INGEST_KEY',
          ...
          <footer className="border-t border-white/[0.06] pt-6 pb-16 flex flex-wrap items-center gap-x-5 gap-y-2 text-[10px] font-mono text-[var(--text-muted)]">
            {[
              { href: 'https://github.com/kandelsecurity/kandeleye', label: 'GitHub' },
              { href: 'https://discord.gg/EPaFD5FFKf', label: 'Discord' },
              { href: 'https://x.com/soulsimplifai', label: 'X' },
              { href: 'https://github.com/kandelsecurity/kandeleye/issues', label: 'Report an issue' },
            ].map(l => (
              <a
                key={l.label}
                href={l.href}
